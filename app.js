(function() {
    ...
    var createExistingGameElement = function(game) {
      var html = '<li><a href="#game?id=' + game.id + '">';
      html += 'שוחק ב- ' + game.last_turn + ' נגד ' + game.played_against;
      html +='</a></li>';
      return html;
    };
        ...
        $.post("shabetz_mila/get_gamelist",
            ...
            function(response) {
                ...
                for (var i=0; i<games.length; i++) {
                    $("#existingGamesList").append(createExistingGameElement(games[i]));
                }
                $("#existingGamesList").children().each(
                    function () {
    ...
    $("#game").on('pageshow', function(event) {
        if (appViewModel.gameId != 0) {
            $.post("shabetz_mila/get_game_by_id", { game_id: appViewModel.gameId },
                function(response) {
                    appViewModel.updateFromServer(response);
                });
        }
